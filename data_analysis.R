...
price_calc <- function(id){
  q <- paste0("SELECT * FROM `products` WHERE type <> ' ' AND `query` = '", id,"'")
  ...
  for(type in unique(r$type)){
  insert_query <- "INSERT INTO product_price (query, condition_own, price, 0_quant, 20_quant, 90_quant, 100_quant, amount, type) VALUES "
    for(i in (10:2)){
      
      idx <- which(r$condition_own>=i-1 & r$condition_own <= i+1 & r$type == type & r$matching != "b")
      data <- r[idx,]
      ...
      insert_query <- paste0(insert_query," (\'",id,"\',",i,",",weighted_price,",", quants[1],",", quants[2],",", quants[3],",",quants[4],",", nrow(data),",'",type, "'),")
    }
